...
function Row({ label, value }: { label: string; value: string }) {
  return (
    <div className="flex items-center justify-between gap-3 border-b border-lavender-mist/40 py-2 text-sm last:border-0">
      ...
      <span className="max-w-[60%] truncate text-right font-medium text-ink-black">{value}</span>
    </div>
  )
}
          ...
          <>
            <DialogHeader>
              <div className="flex items-center gap-3">
                ...
                <DialogTitle className="truncate text-ink-black">{file.originalName}</DialogTitle>
              </div>
            </DialogHeader>
