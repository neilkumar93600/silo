...
export function DashboardPreview() {
  return (
    <div className="relative overflow-hidden rounded-[20px] border border-lavender-mist bg-eclipse-black text-left shadow-[0_0_60px_rgba(185,151,255,0.15)]">
      {/* Top window bar */}
      <div className="flex items-center justify-between border-b border-lavender-mist bg-cream-canvas/60 px-4 py-3">
      ...
      <div className="p-4 sm:p-6">
        <div className="mb-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <span className="text-[14px] font-medium text-ink-black">All Files</span>
            <span className="rounded-full bg-carbon-ink px-2 py-0.5 font-mono text-[10px] text-ash-wisp border border-lavender-mist">
              3 items
            </span>
          </div>
          <div className="rounded-md border border-lavender-mist bg-cream-canvas px-3 py-1 text-[12px] text-silver-smoke">
            Filter: All
          </div>
        </div>

        <div className="flex flex-col divide-y divide-lavender-mist/40 rounded-lg border border-lavender-mist/60 bg-cream-canvas/80">
          {rows.map((row) => (
            <div key={row.name} className="flex items-center justify-between px-4 py-3.5 transition-colors hover:bg-lavender-mist/20">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <span className="truncate text-[14px] font-medium text-ink-black">{row.name}</span>
              </div>
